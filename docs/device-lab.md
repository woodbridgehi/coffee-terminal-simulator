...
选择设备 → 设备配置 → 安装与原点，可调整父安装点、位置和旋转，显示坐标轴并恢复默认安装。精细模型、工位与碰撞体一起更新；夹爪支持相对法兰的安装偏移。详见 [设备原点与安装关系](device-installation.md)。

## 杯垫到位检测与保存反馈

落杯器的圆形杯垫与底板已分层，并按杯底调整高度。杯垫灯环：绿色表示检测到杯子，黄铜色为空垫，灰色为传感器停用或设备离线，设备故障显示红色。选择落杯器后，状态区显示「杯垫检测到杯子」和「杯垫传感器启用」。

配置 `devices.cup-dispenser.cupSensor`：`enabled` 默认 true；`radiusMm` 默认 80（范围 1–145）；`heightToleranceMm` 默认 8（范围 0.1–30）。页面「设备配置」可编辑，JSON 导出/导入保留这些参数。传感器区域跟随设备工作点移动、旋转；默认兼容没有此字段的旧配置。

`GET /api/devices/cup-dispenser` 返回 `sensors.cupPresent`、`sensors.cupSensorEnabled`、`observedAt` 与 `sensorCapabilities.cupPresent`。按工作点局部坐标检测杯中心投影、杯底高度及直立程度；不检测装饰杯堆或落杯预览动画。杯子在原地被夹住仍然有信号，抬离杯垫后才无信号。它是几何到位开关，不模拟真实光电、重量或接触力。信号沿用 sensorDelaySeconds、断连缓存和 forcedSensors 异常注入；不会绕过实际落杯联锁或改变库存。

保存设备配置时，按钮旁显示正在检查、已保存、没有变化或明确的拒绝原因。失败保留原配置及输入，便于修正重试；空数值不会被误当作零保存。顶部通知也保留。当前仍不支持通过鼠标点击模型表面创建安装点，只支持命名安装点和数值编辑。

## 将配置用于独立实验

保存设备配置后，点击「用当前配置打开实验室」。该操作复制配置并创建独立实验，不改变联调台现场。也可导出设备配置文件后在实验室导入。详见 [共享工作台配置](shared-workcell.md)。
