...
import {experimentTasks} from '../coffee-terminal/web/twin/shared-config.mjs';
...
if(args.includes('--replay')) {
  ...
  const sim=await createExperiment(recorded.config,recorded.tasks,{policy:recorded.policy,record:false});
  ...
  const tasks=args.includes('--tasks')?JSON.parse(await readFile(option('--tasks'),'utf8')):experimentTasks(config);
  const runs=[];
  for(const policy of args.includes('--compare')?['serial','parallel']:[option('--policy','parallel')]) {
    const sim=await createExperiment(config,tasks,{policy,record:!!option('--output')});const metrics=sim.run(maxSeconds);runs.push(sim.export());console.log(JSON.stringify({policy,...metrics},null,2));if(metrics.status!=='completed')process.exitCode=1;
  }
