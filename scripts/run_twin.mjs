...
if(args.includes('--help')){console.log('npm run twin:run -- [--world file.json] [--tasks tasks.json] [--policy serial|parallel] [--compare] [--output experiment.json] [--replay experiment.json] [--max-seconds 300]');process.exit(0);}
const maxSeconds=Number(option('--max-seconds',300));if(!Number.isFinite(maxSeconds)||maxSeconds<=0)throw Error('Invalid max-seconds');
if(args.includes('--replay')) {
  ...
  const config=JSON.parse(await readFile(option('--world',new URL('../config/twin/coffee-workcell-v1.json',import.meta.url)),'utf8'));
  const tasks=args.includes('--tasks')?JSON.parse(await readFile(option('--tasks'),'utf8')):latteTasks();
  const runs=[];
