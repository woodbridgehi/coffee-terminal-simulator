...
| 13 | 咖啡终端 | 向同一事件接口依次发送 `task.started`、`step.started`、`task.progress` | `taskId`、`recipeId`、`stepId`、`stepName`、`stepIndex`、`stepCount`、`stepProgress`、`overallProgress`、`remainingSeconds` | `task.started` 立即可靠处理；进度满足整杯变化至少 5% 或最长 5 秒时上报，云端前置层只保留最新进度并批量写入任务快照。订单服务不得自行推导步骤或重新计算随机时长。 |
