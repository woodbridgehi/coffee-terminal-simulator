...
self.onmessage=async({data})=>{
  try {
    if(data.type==='init')sim=await Simulation.create(data.config,data.tasks??latteTasks(data.config),{policy:data.policy??'parallel'});
      ...
      const results=[];
      for(const policy of ['serial','parallel']) {const experiment=await Simulation.create(data.config,latteTasks(data.config),{policy,record:false});results.push({policy,...experiment.run(600)});}
