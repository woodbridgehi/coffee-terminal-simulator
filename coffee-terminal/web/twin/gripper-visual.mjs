...
export function createGripperVisual(){
 const group=new THREE.Group();group.name='parallel-gripper-85mm';
 ...
 box(group,[.110,.060,.020],[0,0,-.0585],silver,.009);
 box(group,[.108,.058,.012],[0,0,-.0745],M.dark,.008);
 ...
 box(group,[.086,.004,.002],[0,0,-.0475],M.dark,.001);
 const jaws=[];
 ...
 const setState=state=>{const width=Math.max(0,Math.min(85,state.openingMm??85))/1000;for(const {jaw,sign} of jaws)jaw.position.x=sign*(width/2+.008);led.material.color.set(state.fault?'#8A5A3B':state.mode==='running'?'#B78A52':'#34795D');group.userData.openingMm=width*1000;};
 setState({openingMm:85});return {group,setState};
}
